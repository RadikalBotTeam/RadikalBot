			...
			local function unwarn_cb(TM, BD)
				...
				if not redis:hget(hashwarn, BD.id_) then
					...
					if not lang then
						...
						return tdcli.sendMessage(TM.chat_id, "", 0, "_تمامی اخطارهای_ "..user_name.." *"..BD.id_.."* *پاک شدند*", 0, "md")
					end
				end
			end
			tdcli_function ({
		...
		if cmd == "warn" then
			...
			if tonumber(warnhash) == tonumber(max_warn) then
				...
				if not lang then
					...
					return tdcli.sendMessage(TM.chat_id, "", 0, "_کاربر_ "..user_name.." *"..BD.id_.."* به دلیل دریافت اخطار بیش از حد اخراج شد\nتعداد اخطارها : "..hashwarn.."/"..max_warn.."", 0, "md")
				end
			else
				redis:hset(hashwarn, BD.id_, tonumber(warnhash) + 1)
				if not lang then
...
local function PursonTeam(msg, matches)
	local hash = "gp_lang:"..msg.to.id
	local lang = redis:get(hash)
	local Chash = "cmd_lang:"..msg.to.id
	local Clang = redis:get(Chash)
	if ((matches[1]:lower() == 'clean' and matches[2] == 'warns' and not Clang) or (matches[1] == "پاکسازی" and matches[2] == 'اخطارها' and Clang)) then
		...
		if not lang then
			...
			return "_تمام اخطارهای کاربران این گروه پاک شد_"
		end
	end
